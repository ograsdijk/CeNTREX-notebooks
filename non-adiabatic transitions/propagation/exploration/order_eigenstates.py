...
def reorder_full(eigvals, eigvecs, reorder, stride):
    s = 0
    idR = 0
    for i in range(1,len(eigvals)):
        if s == stride:
            s = 0
            if i+stride < len(eigvals):
                idR += 1
        eigvals[i] = eigvals[i, reorder[idR]]
        eigvecs[i] = eigvecs[i][:,reorder[idR]]
        s += 1
    return eigvals, eigvecs

def eigenshuffle(Asequence, return_reorder = False):
    """
