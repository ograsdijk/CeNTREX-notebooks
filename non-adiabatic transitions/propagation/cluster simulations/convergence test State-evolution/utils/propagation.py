...
def propagate_exp_HamArray(HamArray, phi0, dt):
    ...
    for idH, H in enumerate(HamArray[:-1]):
        phi[idH+1] = expm(-1j*2*np.pi*H*dt)@phi[idH]
    return phi
...
def propagate_exp_adaptive(ham, phi0, traj, E, B, dtAdaptive = (1e-7, 1e-6)):
    ...
    while ti <= traj[-1]:
        ...
        if (zi > -0.36) & (zi < -0.260):
            ...
            dt = dtAdaptive[1]
        phi = (expm(-1j*2*np.pi*ham(*E(xi,yi,zi), *B(xi,yi,zi))*dt)@phi)
        ti += dt
...
def propagate_exp_adaptive_RF(ham, phi0, traj, E, B, RFfreq, RFamp, dtAdaptive = (5e-9, 1e-6)):
    ...
    while ti <= traj[-1]:
        ...
        phi = (expm(-1j*2*np.pi*ham(Ex,Ey,Ez, *B(xi,yi,zi))*dt)@phi)
        ti += dt
